                    ...
                    steps {
                        script {
                            sh '''
                                ...
                                ansible dev -m ping  --private-key devops-henry.pem  -o 
                            '''
                        }
                    }
                }
                ...
                stage ("DEV - Install Docker on ec2 hosts") {
                    steps {
                        script {
                            ...
                            sh '''
                                export ANSIBLE_CONFIG=$(pwd)/Sources-Ansible/ansible.cfg
                                ansible-playbook Sources-Ansible/playbooks/install-docker.yml --vault-password-file vault.key  --private-key devops-henry.pem -l ic_webapp_server_dev
                            '''                                
                        }
                    }
                }
                ...
                stage ("DEV - Deploy pgadmin") {
                    steps {
                        script {
                            sh '''
                                export ANSIBLE_CONFIG=$(pwd)/Sources-Ansible/ansible.cfg
                                ansible-playbook Sources-Ansible/playbooks/deploy-pgadmin.yml --vault-password-file vault.key --private-key devops-henry.pem -l pg_admin_server_dev
                            '''
                        }
                    }
                }
                ...
                stage ("DEV - Deploy odoo") {
                    steps {
                        script {
                            sh '''
                                export ANSIBLE_CONFIG=$(pwd)/Sources-Ansible/ansible.cfg
                                ansible-playbook Sources-Ansible/playbooks/deploy-odoo.yml --vault-password-file vault.key  --private-key devops-henry.pem -l odoo_server_dev
                            '''
                        }
                    }
                }
            ...
            steps {
                script {       
                    ...
                    sh'''
                        cd "./Sources-Terraform/app"
                        terraform destroy --auto-approve
                        rm -rf Sources-Ansible/host_vars/*.dev.yml
                        rm -rf devops-henry.pem
                    '''                            
                }
            }
        }  
        ...
        always {
            script {
                /*sh '''
                    ...
                    rm -f vault.key id_rsa id_rsa.pub password devops-henry.pem public_ip.txt
                ''' */
